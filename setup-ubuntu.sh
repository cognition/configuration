...
if [[ ! -f /.bootstraped ]]; then touch /.bootstraped ; fi
...
cp -f ${REPO_PATH}/files/etc-bash/* /etc/bash.bashrc.d/ ; cp -f ${REPO_PATH}/completions/* /etc/bash_completion.d/ ; cp -n ${REPO_PATH}/files/home/bashrc ${SKEL}/.bashrc ; cp -n ${REPO_PATH}/files/home/over-ride ${SKEL}/.over-ride; cp -Rn ${REPO_PATH}/files/home/vim ${SKEL}/.vim; cp -n ${REPO_PATH}/files/home/tmux.conf ${SKEL}/.tmux.conf ;cp -n ${REPO_PATH}/files/home/vimrc ${SKEL}/.vimrc; install -m 755 -o root -C src/koolify-user.sh /usr/local/bin/koolify-user
